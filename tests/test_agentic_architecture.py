...
class AgenticArchitectureTests(unittest.TestCase):
    ...
    def test_planner_preserves_rich_backend_scene_descriptions(self):
        ...
        self.assertIn("cozy moonlit nursery", plan.scenes[0].video_prompt)
    ...
    def test_planner_normalizes_scenes_as_distinct_storyboard_cuts(self):
        ...
        self.assertTrue(all("StoryMem keyframe memory" in scene.video_prompt for scene in plan.scenes))
        self.assertTrue(all("No dialogue or background music" in scene.video_prompt for scene in plan.scenes))
