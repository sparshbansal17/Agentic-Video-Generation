...
def rope_apply(x, grid_sizes, freqs, memory_size):
    ...
    for i, (f, h, w) in enumerate(grid_sizes.tolist()):
        ...
        x_i = torch.view_as_complex(x[i, :seq_len].to(torch.float64).reshape(
            seq_len, n, -1, 2))
        # idx_t = list(range(-memory_size * MEMORY_ROPE_SHIFT, 0, MEMORY_ROPE_SHIFT)) \
        #         + list(range(0, f - memory_size))
        # idx_t = [idx + ROPE_MAX_LEN // 2 for idx in idx_t]
        pos_t = torch.cat([
...
class WanSelfAttention(nn.Module):
    ...
    def forward(self, x, seq_lens, grid_sizes, freqs, memory_size):
        r"""
        Args:
...
class WanAttentionBlock(nn.Module):
    ...
    def forward(
        ...
        y = self.self_attn(
            ...
            seq_lens, grid_sizes, freqs, memory_size)
        with torch.amp.autocast('cuda', dtype=torch.float32):
            x = x + y * e[2].squeeze(2)
